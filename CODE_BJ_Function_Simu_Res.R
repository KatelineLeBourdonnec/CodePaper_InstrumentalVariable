...
# 1st stage and naive and residual estimation techniques. 
# The substitution estimation technique is given in file 
# "CODE_BJ_Function_Simu_Log-Sub.R". 
# The linear first-stage alternative is given in file
# "CODE_BJ_Function_Simu_Linear.R"
...
#The results of this script are output in folder "log_res". 
...
job <- 9754976 # TO BE CHANGED BY THE USER (if not run on a computing server)
...
# estimation
hlme_N <- hlme(formulaY_naif0,random=~ 1+Temps, subject="ID",data=Data_num)
summary <- summary(hlme_N)
CoefNaif <- hlme_N$best[[3]]
CoefNaifInt <- hlme_XeHat$best[[4]]
sd2Naif <- summary[,2][[3]]
sd2NaifInt <- summary[,2][[4]]
...
Data_num$resid <- residuals.glm(glmfit0,type = "working")
...
# 2nd stage model 
# formula
formulaY_2stage <- Y~Temps*(Xe + resid)
...
Coef2stageInter <- hlme_XeHat$best[[3+2]]
sd2stageM <- summary[,2][[3]]
sd2stageInter <- summary[,2][[3+2]]
...
res <- list(CoefNaifInt,CoefNaif,sd2Naif,sd2NaifInt,Coef2stageM,Coef2stageInter,sd2stageM,sd2stageInter,R2_0,var_corrig,se_corr,TC,TCint,VarIntra,VarInter,VarTot,VarIntraInt,VarInterInt,VarTotInt,resu[[1]],resu[[2]])

##### Output #####
save(res,file=paste("simu_boots_2sri_2000_ALPHA2_",job,"_",rep,".Rdata",sep=""))
