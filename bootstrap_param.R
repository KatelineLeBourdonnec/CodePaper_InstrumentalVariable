#### Function for bootstrap for logistic first stage: #### 
...
bootstrap_param <- function(K,data_suj,data_num){
  ...
  glmfit <- glm(Xe ~ Xiv,data=data_suj, family="binomial")
  res <- summary(glmfit)
  coefAlpha <- glmfit$coefficients
  sdAlpha <- res$coefficients[,2]
  
  # STEP 2 : Generate data according to Xhat value 
  B <- NULL
  alphaB <- NULL
  n <- NULL
  for(i in 1:length(coefAlpha)){
    B <- rnorm(n=K,coefAlpha[i],sdAlpha[i])
    alphaB <- cbind(B, alphaB)
    name <- paste("alpha",i,sep = "_")
    n <- append(n,name)
  }
  colnames(alphaB) <- n
  ...
  for(i in 1:K){
    ...
    for(j in unique(data_num$ID)){ 
      ...
      #VarCorr <- (as.matrix(t(X))%*%Vi1*as.vector(VY))%*%Vi1%*%as.matrix(X)
